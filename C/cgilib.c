...
#ifdef CGI_DEBUG
void *cgi_alloc(size_t size, int flag, const char *src, int line)
#else
...
#endif
{
	void *ret;
#ifdef CGI_DEBUG
	if(flag && (ret=_dbg_malloc(size, line, src)) == NULL){
		(*cgi_badalloc_func)();
		exit(3);
	}
#else
...
int str_icomp(const char *str1, const char *str2)
{
	...
	CGI_ASSERT(p1!=NULL && p2!=NULL, malloc_error);
	ret = strcmp(p1,p2);
	free(p1);
	free(p2);
	return ret;
...
int ipv4addr2host(const char *ipv4, char *host, size_t len)
{
	...
	/*
	if(he->h_aliases[0] != NULL) strncpy(host, he->h_aliases[0], len);
	else strncpy(host, he->h_name, len);
	*/
				...
				if(rbuf[rs-1] == 0x0d){
					...
					if(rbuf[rs] != 0x0a){
						...
						/* if(fseek(stdin,-1,SEEK_CUR)){
							CGILIB_TMPDEL(pftmp);
							cgi_errendex(__LINE__, __FILE__, "fseek error");
						*/
						crpadding = 1;
					}else{
						++rs;
...
{
	put_html_header();
	puts("<b><font color=\"#ff0000\">malloc error</font></b>");
	exit(1);
}
